...
class xbs_tagstagsListHandler extends XoopsObjectHandler {
  ...
  */
   ...
   *
   * @param Object    tagsList Object to delete
   * @return bool TRUE on success else False
   */
  function delete(&$obj) {
    $sql = sprintf("delete from %s where id = %u",$this->db->prefix(TAGS_TBL_LIST),intval($obj->getVar('id')));
    if(!$result = $this->db->queryF($sql)) {
      $this->setError($this->db->errno(),$this->db->error());
      return false; 
    } else {
