...
function adminSelectPage() {
	//initiate the form
	$cols = array (_AM_FRM1_COL1,_AM_FRM1_COL2,_AM_FRM1_COL3,_AM_FRM1_COL4);
	$table = new tagsTableForm($cols,null,false,TAGS_URL."/admin/index.php?new=1",TAGS_URL."/admin/index.php?edit=",TAGS_URL."/admin/index.php?delete=");
	$tagsHandler =& xoops_getmodulehandler("tagsPage");
	$tags =& $tagsHandler->getAllPages();
	foreach ($tags as $tag) {
		$row = array($tag->getVar('id'),$tag->getVar('tags_modname'),$tag->getVar('tags_fname'),$tag->getVar('tags_config'));
		$table->addRow($row);
	}
	$table->display();
}//end function
...
*
* @version 1
* @param string $id id of page record to edit, if 0 then new page
*/
	...
	$tagsHandler =& xoops_getmodulehandler("tagsPage");
  	if ($id==0) {
  		...
  		if ($result = $xoopsDB->query($sql)) {
		  	...
		  	$tags->setVar('pid',intval($res['pid'])+1);
  		} else {
  			$tags->setVar('pid',0);
  		}
	  	$tags->setVar('mid',$mid);
...
function adminDeletePage($id) {
	$id = intval($id);  //check it is an integer
	if ($id > 0) {
		$tagsHandler =& xoops_getmodulehandler("tagsPage");
		$tags =& $tagsHandler->get($id);
...
function adminSelectUpdate() {
  ...
  if (count($fmodule->_options)>0) { 
	  ...
	  $editForm = new XoopsThemeForm(_AM_FRM3_TITLE,"pageform","update.php");  $editForm->addElement($fmodule);
	  $editForm->addElement($ftray);
	  ...
	  if (file_exists($fname)) {  //get info from configuration file
	  	...
	  	if (isset($metatags)) {
	  		...
	  		foreach ($metatags as $page) {
	 			...
	 			$sql = sprintf("insert into %s (mid,pid,tags_title,tags_fname,tags_desc,tags_config,tags_keyword,tags_minkeylen,tags_maxkeyword) values (%u,%u,%s,%s,%s,%s,%s,%u,%u)",$xoopsDB->prefix(TAGS_TBL_TAGS),$mid,$pid,$xoopsDB->quoteString($page['title']),$xoopsDB->quoteString($page['script_name']),$xoopsDB->quoteString($page['description']),$xoopsDB->quoteString($page['config']),$xoopsDB->quoteString($page['keywords']),$page['minkeylen'],$page['maxkeys']);
	  	...
	  	return true;
	  } else {  //do manual search of system for data
		  //set the module filename
		  $sql = "insert into ".$xoopsDB->prefix(TAGS_TBL_TAGS)." (mid, tags_title, tags_fname) select t1.mid, t1.name, concat('/modules/',t1.dirname,'/index.php') from ".$xoopsDB->prefix("modules")." as t1 where t1.mid=".intval($mid);
		  if (!$result = $xoopsDB->queryF($sql)) {
		  	return false;
		  }
		   ...
		   */
		  $sql = "select tags_fname,tags_title from ".$xoopsDB->prefix(TAGS_TBL_TAGS)." where mid =".intval($mid);
		  ...
		  $arr = $xoopsDB->fetchArray($mainresult);
	    	...
	    	if (file_exists($fname)) {
	    		...
	    		if (isset($modversion['hasMain']) && $modversion['hasMain']==1) {  //has userside menu so process
	    			if (isset($modversion['sub'])) {
	    				foreach ($modversion['sub'] as $menu) {
	    					...
	    					if (file_exists(XOOPS_ROOT_PATH.$fname)) {
	    						$sql = sprintf("insert into ".$xoopsDB->prefix(TAGS_TBL_TAGS)." (mid, pid, tags_fname, tags_title, tags_desc) values (%u, %u, %s, %s, %s)", $mid, $pid, $xoopsDB->quoteString($fname), $xoopsDB->quoteString($arr['tags_title']." - ".$menu['name']), $xoopsDB->quoteString($modversion['description']));
	    						if (!$result = $xoopsDB->queryF($sql)) {
	  								return false;
	    						}
	    						$pid ++;
...
function adminSelectTrack() {
	//initiate the form
	$cols = array (_AM_FRM6_COL1,_AM_FRM6_COL2,_AM_FRM6_COL3,_AM_FRM6_COL4);
	$table = new tagsTableForm($cols,null,true,null,null,TAGS_URL."/admin/tracks.php?delete=");
	$tagsHandler =& xoops_getmodulehandler("tagsPage");
	$trackHandler =& xoops_getmodulehandler("tagsTrack");
	$tags =& $tagsHandler->getAllPages();
	foreach ($tags as $tag) {
		$numTracks = $trackHandler->countTracks(intval($tag->getVar('id')));
		$script = ($numTracks>0?"<a href='tracks.php?view=".$tag->getVar('id')."'>".$tag->getVar('tags_fname')."</a>":$tag->getVar('tags_fname'));
		$row = array($tag->getVar('id'),$tag->getVar('tags_modname'),$script,$numTracks);
		$table->addRow($row);
	}
	$table->display();
